...
class SRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        max_x = hr_w - self.patch_size
        max_y = hr_h - self.patch_size
        ...
        hr_patch = hr_img_t[y : y + self.patch_size, x : x + self.patch_size]
        ...
        if hr_patch.shape[0] < self.patch_size or hr_patch.shape[1] < self.patch_size:
            pad_h = max(0, self.patch_size - hr_patch.shape[0])
            pad_w = max(0, self.patch_size - hr_patch.shape[1])
...
def plot_training_history(train_losses, val_losses):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss (MSE)')
    plt.title('ESPCN Training History')
    plt.legend()
    plt.grid(True)
...
if __name__ == "__main__":
    ...
    train_lr_dir = "resources/datasets/DIV2K/DIV2K_train_LR_bicubic/X2"
    valid_hr_dir = "resources/datasets/DIV2K/DIV2K_train_HR"
    valid_lr_dir = "resources/datasets/DIV2K/DIV2K_train_LR_bicubic/X2"
